...
class RegisterPageBloc extends Bloc<RegisterPageEvent, RegisterPageState> {
  RegisterPageBloc({required CreateUserUseCase createUserUsecase})
    ...
    on<EditPasswordEvent>(
      _changePassword,
      transformer: _debounceEvent<EditPasswordEvent>,
    );
    on<EditConfimrationPasswordEvent>(
      _editConfirmPass,
  ...
  final CreateUserUseCase
      _getRegisterUseCase; // Приватное поле для экземпляра CreateUserUseCase

  Future<void> _changePassword(
    EditPasswordEvent event, // Функция для изменения пароля
    StateEmitter emit,
  ) async {
    final passVm =
        state.passwordViewModel; // Получаем текущий viewModel для пароля
    emit(
      state.copyWith(
        // Изменяем состояние, обновляя значение пароля в viewModel
  ...
  Future<void> _editEmail(EditEmailEvent event, StateEmitter emit) async {
    ...
    emit(
      state.copyWith(
        emailViewModel: newEmailTextViewModel,
      ),
    ); // Изменяем состояние, устанавливая новый viewModel для email
  }

  Future<void> _editConfirmPass(
    EditConfimrationPasswordEvent
        event, // Функция для изменения повторного ввода пароля
    Emitter<RegisterPageState> emit,
  ) async {
    final password = event
        .confirmationPassword; // Получаем значение повторного ввода пароля из события
    final viewModel = state
        .repeatPasswordFromViewModel; // Получаем текущий viewModel для повторного ввода пароля
    emit(
      state.copyWith(
        // Изменяем состояние, обновляя значение повторного ввода пароля в viewModel
        repeatPasswordFromViewModel: viewModel.copyWith(
          value:
              password, // Устанавливаем новое значение повторного ввода пароля
        ),
      ),
    );
  }
  ...
  Future<void> _submit(
    SendDataEvent event, // Функция для отправки данных
    Emitter<RegisterPageState> emit,
  ) async {
    emit(
      state.copyWith(
        // Изменяем состояние, устанавливая статус загрузки
        status: RegistrationStatus.loading,
      ),
    );
    final validEmail = isEmailValid(emit); // Проверяем валидность email
    final isPasswordValid =
        isPasswordsValid(emit); // Проверяем валидность пароля
    final bool isAllDataIsValid =
        isPasswordValid && validEmail; // Проверяем, что все данные валидны

    if (!isAllDataIsValid) {
      // Если не все данные валидны
      emit(
        state.copyWith(
          // Изменяем состояние, устанавливая статус ошибки и сообщение
      ...
      (right) => // Если операция успешна
        ...
        state.copyWith(
          status: RegistrationStatus
              .succeed, // Изменяем состояние, устанавливая статус успешной авторизации и сообщение
  ...
  bool isPasswordsValid(Emitter<RegisterPageState> emit) {
    ...
    final isValid = passwordRegex.hasMatch(
      state.passwordViewModel.value,
    ); // Проверяем пароль по регулярному выражению
    ...
    final isIdentical =
        passVm.value == confirmPassVm.value; // Проверяем совпадение паролей
    ...
    emit(
      state.copyWith(
        // Изменяем состояние, обновляя viewModel для пароля и повторного ввода пароля
        passwordViewModel: passVm.copyWith(
          isValid: isValid,
    ...
    return isValid &&
        isIdentical; // Возвращаем true, если пароли валидны и совпадают
  }
  ...
  bool isEmailValid(Emitter<RegisterPageState> emit) {
    ...
    final isValid = emailRegex.hasMatch(
      state.emailViewModel.value,
    ); // Проверяем email по регулярному выражению

    if (!isValid) {
      // Если email не валиден
      final emailVm = state.emailViewModel.copyWith(
        // Создаем новый viewModel с сообщением об ошибке
        isValid: isValid,
      ...
      ); // Изменяем состояние, обновляя viewModel для email
    }
  ...
  Stream<T> _debounceEvent<T extends RegisterPageEvent>(
    Stream<T> stream, // Функция для добавления задержки к событиям
    Stream<T> Function(T) s,
  ) =>
      stream
          .debounceTime(const Duration(milliseconds: 500))
          .asyncExpand(s); // Применяем задержку к событиям
}
